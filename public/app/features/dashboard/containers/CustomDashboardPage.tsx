...
function mapStateToProps(state: StoreState, props: OwnProps) {
  const dashtype = props.queryParams.dashtype ? parseInt(props.queryParams.dashtype, 10) : 0;
  const siteId = props.queryParams.siteId ? parseInt(props.queryParams.siteId, 10) : 0;
  const assetId = props.queryParams.assetId ? parseInt(props.queryParams.assetId, 10) : 0;

  const loadingNav = getDashboardNav(
    `${props.match.params.slug}`,
    dashtype,
    siteId,
    assetId,
    state.dashboardNavs.dashboardNavs
  );
...
export type Props = Themeable2 & OwnProps & ConnectedProps<typeof connector>;
...
export class UnthemedCustomDashboardPage extends PureComponent<Props, State> {
  ...
  async fetchDashboards() {
    const { loadDashboards, folder, siteId, assetId } = this.props;
    await loadDashboards(folder, siteId, assetId);
  }
  ...
  render() {
    const { dashboard, isInitSlow, initError, queryParams, theme, navModel } = this.props;
...
export const CustomDashboardPage = withTheme2(UnthemedCustomDashboardPage);
CustomDashboardPage.displayName = 'CustomDashboardPage';
export default connector(CustomDashboardPage);
